...
exports.color = {
    ...
    parse: (context, value) => {
        ...
        if (value.type === 20 /* TokenType.IDENT_TOKEN */) {
            const namedColor = exports.COLORS[value.value.toUpperCase()];
...
const rgb = (_context, args) => {
    const tokens = args.filter(parser_1.nonFunctionArgSeparator);
    if (tokens.length === 3) {
...
const hsl = (context, args) => {
    ...
    const h = (hue.type === 17 /* TokenType.NUMBER_TOKEN */ ? (0, angle_1.deg)(hue.number) : angle_1.angle.parse(context, hue)) / (Math.PI * 2);
    const s = (0, length_percentage_1.isLengthPercentage)(saturation) ? saturation.number / 100 : 0;
    const l = (0, length_percentage_1.isLengthPercentage)(lightness) ? lightness.number / 100 : 0;
    const a = typeof alpha !== 'undefined' && (0, length_percentage_1.isLengthPercentage)(alpha) ? (0, length_percentage_1.getAbsoluteValue)(alpha, 1) : 1;
    if (s === 0) {
...
const parseColor = (context, value) => exports.color.parse(context, parser_1.Parser.create(value).parseComponentValue());
